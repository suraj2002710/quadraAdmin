...
const Wallet = () => {
    ...
    const getwallet = async () => {
      const res = await axios.post(`${baseurl}/admin/fetch_all_client_wallet/`, {
    ...
    const sendpageinquery=(val)=>{
      navigate({
        pathname:'/dashboard/clientwallet',
        search:`?page=${val}`
      })
    }
  ...
  const handleClose = () => {
    ...
    const walletdelete = async (id) => {
      ...
      const res = await axios.post(`${baseurl}/admin/delete_client_wallet_by_client_id/`, {
    ...
    const walletingle_fetch = async (id) => {
      const { data } = await axios.post(`${baseurl}/admin/fetch_client_wallet_by_client_id/`, {
        ...localClients, client_id: id
      })
      console.log(data);
      if (data.status === "Success") {
        console.log(data);
        setwallet_id(data.data[0].client_id)

        seteditSpend(data.data[0].spend)
      }
    }
    ...
    const wallet_edit = async (e) => {
      ...
      const { data } = await axios.post(`${baseurl}/admin/update_client_wallet_by_client_id/`, {
        ...localClients, client_id: wallet_id, spend:editSpend
      })
      console.log(data);
      if (data.status === "Success") {
        seteditmodalShow(false)
    ...
    const searchfilter = async (query) => {
      setloading(true)
      const { data } = await axios.post(`${baseurl}/admin/search_client_wallet/`, {
                    ...
                    <div className="table-chat">
                      <div className="table-chat-body table-responsive p-0">
                        <table id="result_list" className="table table-quadra">
                          ...
                          {
                            wallet.map((curEle, i) => {
                              const { id, client_name,spend,client_id } = curEle
                              // console.log(rating);
                        ...
                        >
                          ...
                          <Modal.Body>
                            ...
                            {/* <label id='labelsinmodal'>Id</label>
                              <input id='Footerlinks1' type="text" value={wallet_id} disabled />
                              <label id='labelsinmodal'>point</label>
                              
   */}
                              <label id='labelsinmodal'>Spend</label>
                              {/* <input type="number" placeholder='Enter Link here' value={editprice} name='media_link' onChange={(e) => {
                                                                    seteditprice(e.target.value)
                                                                    setpriceErr('none')
                                                                }} /> */}
                              <input id='Footerlinks1' type="number" value={editSpend} onChange={(e) => seteditSpend(e.target.value)} placeholder="Enter Spend" />
