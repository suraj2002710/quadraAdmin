...
const Notification = () => {
  ...
  useEffect(() => {
    ...
    axios.post(`${baseurl}/admin/all-notifications/`, { ...localClients, user_role: "professional", page: page, page_size: 10 })
      .then(res => {
  ...
  const sendpageinquery = (val) => {
    navigate({
      pathname: '/dashboard/notification/professional',
      search: `?page=${val}`
    })
  }
  ...
  const searchfilter = async (query) => {
    setloading(true)
    const { data } = await axios.post(`${baseurl}/admin/search_notification_professional/`, {
